...
class My2048Env(gym.Env):
    ...
    def __init__(self):
        ...
        # Set negative reward (penalty) for illegal moves (for experiment)
        self.set_illegal_move_reward(-25.)
    ...
    def step(self, action): 
        ...
        try:
            ...
            # Add reward according to weighted states (for experiment)
            ...
            weighted_matrix = self.Matrix * weight
            weighted_sum = np.sum(weighted_matrix)
            reward += weighted_sum

            # reward action
            action2reward = [2.5, 5, 5, 1]
            reward += action2reward[action]
            ...
            # Modify this part for the agent to have a chance to explore other actions (for experiment)
            self.foul_count += 1

            done = True if self.foul_count == 10 else False 
